...
// func (l *DiskLocation) unmountEcVolumeByCollectionAndTime(collectionName string, fromTime uint64, toTime uint64) map[needle.VolumeId]*erasure_coding.EcVolume {
// 	deltaVols := make(map[needle.VolumeId]*erasure_coding.EcVolume, 0)
// 	for k, v := range l.ecVolumes {
// 		if v.Collection == collectionName && uint64(v.CreatedAt().Unix()) >= fromTime && uint64(v.CreatedAt().Unix()) <= toTime {
// 			log.Println("QUYNGUYEN: unmountEcVolumeByCollectionAndTime", v.VolumeId)
// 			deltaVols[k] = v
// 		}
// 	}
