...
type Topology struct {
	...
	//QUYNGUYEN
	chanOutDatedVolumes chan storage.VolumeInfo
...
func NewTopology(id string, seq sequence.Sequencer, volumeSizeLimit uint64, pulse int, replicationAsMin bool) *Topology {
	...
	//QUYNGUYEN
	t.chanOutDatedVolumes = make(chan storage.VolumeInfo)
...
func (t *Topology) DeleteCollection(collectionName string) {
	log.Println("QUYNGUYEN: Topology DeleteCollection filter received: ", collectionName)
...
func (t *Topology) DeleteLayout(collectionName string, rp *super_block.ReplicaPlacement, ttl *needle.TTL, diskType types.DiskType) {
	...
	if len(collection.storageType2VolumeLayout.Items()) == 0 {
		log.Println("QUYNGUYEN: delete collection 4", collectionName)
