...
func (fs *FilerServer) filerHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodDelete:
		fmt.Println("QUYNGUYEN: Handle delete request")
		if _, ok := r.URL.Query()["tagging"]; ok {
			fs.DeleteTaggingHandler(w, r)
		} else if _, ok := r.URL.Query()["namePattern"]; ok {
			// 	// fs.DeleteNamePatternHandler(w, r)
		} else if _, ok := r.URL.Query()["time"]; ok {
			fs.DeleteTimeHandler(w, r)
		} else {
