...
// func (ms *MasterServer) deleteCollectionVolume(collectionName string, fromTime int32, toTime int32) error {
...
// 			return err
// 		}
// 		vls := collection.GetAllVolumeLayouts()
// 		vss := collection.ListVolumeServers()
// 		log.Println("QUYNGUYEN: delete collection ", collectionName, len(vls), len(vss))
// 	}
// 	return nil
// }
...
func (ms *MasterServer) volumeDeleteHandler(w http.ResponseWriter, r *http.Request) {
	count := uint64(0)
	option, err := ms.getVolumeGrowOption(r)
	if err != nil {
		writeJsonError(w, r, http.StatusNotAcceptable, err)
